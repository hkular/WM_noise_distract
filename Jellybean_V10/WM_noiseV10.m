...
% Make sure psychtoolbox is on your path addpath(genpath('/Applications/Psychtoolbox'))
% Inputs
% p parameter
% info subject info
...
% distractor: orientation w/noise - present or absent
 ...
 % Experiment timing
 % Entire duration: 38.7 mins
 % Block duration: 4.3 mins
 % Task: orientation full report
%% 
function WM_noiseV10(p, info, nruns, startRun)
...
cd(datadir); 
    if exist(['WM_noiseV10_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'])
        load(['WM_noiseV10_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat']);
        ...
        designMat = fullfact([4 4 3 2]); % 4 ori bins and 4 distractor orient 3 distractor present absent change and 2 kappa bandwidths
        designMat = repmat(designMat,3);
...
% full report spin a line, in quadrant we are probing
        ...
        while RespTimePassed<t.ResponseTime  %As long as no correct answer is identified
            ...
            elseif keyCode(KbName('ESCAPE')) % If user presses ESCAPE, exit the program.
                ...
                eval(['save(''WM_noiseV10_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'', ''TheData'', ''TrialStuff'', ''-V7.3'')']);
                cd(expdir)
                Screen('CloseAll');
                ListenChar(1);
