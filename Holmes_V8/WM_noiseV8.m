%% %%  from RR WM_DistractV8_Main_Practice.m adapted by HK, 2023, for HK Holmes
...
%% 
function WM_noiseV8(p, info, nruns, startRun)
...
p.escape = KbName('q');
 ...
 %Screens = Screen('Screens'); %look at available screens
    ...
    p.MyGrey=round((p.black+p.white)/2); % msde it darker
    %p.MyGrey = 100;
...
cd(datadir); 
    if exist(['WM_noiseV8_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'])
        load(['WM_noiseV8_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat']);
...
%Timing params -- 
    ...
    p.nDistsTrial = 15; % 75 frames 15 dists = 5 reps
    t.DistractorTime = 3;% was 2.95
    t.distDur = 1/25; % make it faster 25Hz
    % do a thing to make is arrythmic
    distmat = [1;0];
    distmat = repmat(distmat, 37,1); % 2*37 = 74, and then +1 since always start on 1
    t.DistArray = Shuffle(distmat); % 1s and 0s to delete random frames, 1 = flip, 0 = keep fram
    t.DistArray = [1;t.DistArray];% make sure first num is 1, can't start on a repeat frame  
...
for n = 1:p.NumTrials
     ...
     for d = 1:p.nDistsTrial
       ...
       % this is the old way of making sequence
%         dd=[];
%            for k=1:(t.DistractorTime/t.distDur)
%                dd=[dd;randperm(p.nDistsTrial)']; % cylce through many times no repeats
%            end
        % this is the new way of making sequence 
           dd = [];
           Dists = repmat((1:p.nDistsTrial),1,5);
           for k = 1:(t.DistractorTime/t.distDur)
               
               dd = [dd;Dists(sum(t.DistArray(1:k)))];
...
% full report spin a line, in quadrant we are probing
        ...
        while RespTimePassed<t.ResponseTime  %As long as no correct answer is identified
            ...
            elseif keyCode(KbName('ESCAPE')) % If user presses ESCAPE, exit the program.
                ...
                eval(['save(''WM_noiseV8_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'', ''TheData'', ''TrialStuff'', ''-V8.3'')']);
                cd(expdir)
                Screen('CloseAll');
                ListenChar(1);
