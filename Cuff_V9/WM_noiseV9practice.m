...
%% 
function WM_noiseV9practice(p, info, nruns, startRun)
...
cd(datadir); 
    if exist(['WM_noiseV9practice_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'])
        load(['WM_noiseV9practice_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat']);
   ...
   % set first two trials as distractor flashes
   TrialStuff(1).distractor = 1;
   TrialStuff(1).distractortask = 1;
   TrialStuff(2).distractor = 1;
   TrialStuff(2).distractortask = 1;
...
% full report spin a line, in quadrant we are probing
        ...
        while RespTimePassed<t.ResponseTime  %As long as no correct answer is identified
            ...
            elseif keyCode(KbName('ESCAPE')) % If user presses ESCAPE, exit the program.
                ...
                eval(['save(''WM_noiseV9practice_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'', ''TheData'', ''TrialStuff'', ''-V7.3'')']);
                cd(expdir)
                Screen('CloseAll');
                ListenChar(1);
    ...
    if TrialStuff(n+1).distractortask == 1        
        ...
        data.ChangeFrame(n+1,1) = data.ChangeFrame(n+1,2)-1;data.ChangeFrame(n+1,p.nchangeframes) = data.ChangeFrame(n+1,2)+1;
        delta = randsample([-1,1],1);    % randomly select increment or decrement   
