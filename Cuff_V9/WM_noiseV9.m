...
% TODO
% fix timing problems
% fix distractor flicker rate
% add distractor task:
%   contrast change 0 1
%   discrimination task - press when there
%   staircase increments .1 aim for 75% accuracy 
...
%% 
function WM_noiseV9(p, info, nruns, startRun, TrialStuff)
...
% full report spin a line, in quadrant we are probing
        ...
        % DISTRACTOR for next trial
         ...
         if TrialStuff(n+1).distractortask == 1
             whichframes = (round(p.nDistFrames/3):2*round(p.nDistFrames/3));
             data.ChangeFrame(n+1) = randsample(whichframes,1); % randomly select which frame in the middle second of the task to change
             delta = randsample([-1,1],1);    % randomly select increment or decrement
             data.DistChange(n+1) = TrialStuff(n+1).distractor + (TrialStuff(n+1).distractor*p.stairstep*delta);
         end
    ...
    check = (sum(~isnan(data.DistResp(ind))))/(numel(ind))*100; % check what percent did we catch
    if check > 77
